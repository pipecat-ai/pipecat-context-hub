...
def _request(jsonrpc: Any, request_id: int, method: str, params: dict[str, Any] | None = None) -> Any:
...
def _build_server() -> Any:
    ...
    tools = [
        types.Tool(name="echo", description="Echo a UTF-8 string.", inputSchema={"type": "object", "properties": {"text": {"type": "string"}}}),
        types.Tool(name="validate", description="Require an integer.", inputSchema={"type": "object", "properties": {"count": {"type": "integer"}}, "required": ["count"]}),
    ...
    async def on_call_tool(_ctx: Any, params: types.CallToolRequestParams) -> types.CallToolResult:
        ...
        if params.name == "echo":
            return types.CallToolResult(content=[types.TextContent(text=str(arguments.get("text", "")))])
    ...
    return Server("mcp-2x-probe", version="probe", on_list_tools=on_list_tools, on_call_tool=on_call_tool, on_ping=on_ping)
...
async def _memory_round_trip() -> dict[str, Any]:
    ...
    async with anyio.create_task_group() as task_group:
        task_group.start_soon(server.run, server_receive, server_send, server.create_initialization_options())
        await send(_request(jsonrpc, 1, "initialize", {"protocolVersion": "2025-06-18", "capabilities": {}, "clientInfo": {"name": "probe", "version": "probe"}}))
        ...
        await send(_request(jsonrpc, 3, "tools/call", {"name": "echo", "arguments": {"text": _UTF8_TEXT}}))
        echo = await receive_response(3)
        await send(_request(jsonrpc, 4, "ping"))
        ping = await receive_response(4)
        errors: dict[str, dict[str, Any]] = {}
        for request_id, label, name, arguments in ((100, "unknown_tool", "missing", {}), (101, "generic_exception", "generic_error", {}), (102, "validation_error", "validate", {"count": "not-an-int"})):
            await send(_request(jsonrpc, request_id, "tools/call", {"name": name, "arguments": arguments}))
    ...
    assert ping["result"] == {}
    assert errors["unknown_tool"] == {"jsonrpc": "2.0", "id": 100, "error": {"code": 0, "message": "Unknown tool: missing"}}
    assert errors["generic_exception"] == {"jsonrpc": "2.0", "id": 101, "error": {"code": 0, "message": "registered generic probe failure"}}
    assert errors["validation_error"]["error"] == {"code": -32602, "message": "Invalid request parameters", "data": ""}
    return {"frames": frames, "initialize": initialize, "tools": tools, "echo": echo, "ping": ping, "errors": errors}
...
async def _stdio_round_trip() -> dict[str, Any]:
    ...
    incoming = io.TextIOWrapper(os.fdopen(incoming_read_fd, "rb", buffering=0), encoding="utf-8", newline="\n")
    ...
    requests = [_request(jsonrpc, 1, "initialize", {"protocolVersion": "2025-06-18", "capabilities": {}, "clientInfo": {"name": "probe", "version": "probe"}}), _notification(jsonrpc, "notifications/initialized"), _request(jsonrpc, 3, "tools/call", {"name": "echo", "arguments": {"text": _UTF8_TEXT}})]
    ...
    async with stdio_server(stdin=async_in, stdout=async_out) as (read_stream, write_stream):
        ...
        async with anyio.create_task_group() as task_group:
            task_group.start_soon(server.run, read_stream, write_stream, server.create_initialization_options())
            task_group.start_soon(anyio.to_thread.run_sync, write_requests)
            for _ in range(500):
                await anyio.sleep(0.01)
                if b'"id":3' in outgoing_buffer.getvalue() and _UTF8_TEXT.encode("utf-8") in outgoing_buffer.getvalue():
                    break
            else:
    ...
    assert any(frame.get("id") == 3 and frame.get("result", {}).get("content", [{}])[0].get("text") == _UTF8_TEXT for frame in frames), frames
    assert all("\ufffd" not in line for line in lines)
    result = {"frames": frames, "raw_utf8": raw_output.decode("utf-8"), "encoding": {"stdin": incoming.encoding, "stdout": outgoing.encoding}, "utf8_bytes": _UTF8_TEXT.encode("utf-8").hex(), "caller_owns_wrappers_after_context": not incoming.closed and not outgoing.closed, "stdio_objects_unchanged": __import__("sys").stdin is stdin_before and __import__("sys").stdout is stdout_before}
...
async def _probe_once() -> dict[str, Any]:
    ...
    return {"package": package, "api": {"server_constructor": str(inspect.signature(Server)), "run": str(inspect.signature(Server.run)), "initialization_options": str(inspect.signature(Server.create_initialization_options))}, "tool_constructor": "mcp.types.Tool(name=..., inputSchema=...) accepted", "memory": await _memory_round_trip(), "stdio": await _stdio_round_trip(), "watchdog": {"probe_scope": "SDK transport lifecycle only; hub watchdog remains test_serve_lifetime.py", "graceful_unwind": "server.run returned after explicit stdio EOF"}}
...
def _run_matrix(script: Path) -> int:
    ...
    for label, requirement in specs:
        command = ["uv", "run", "--isolated", "--with", requirement, "--with", "anyio", "--with", "pydantic", "--with", "packaging", "python", str(script), "--single"]
