...
| **R6 home-path redaction** | ⚠️ **GAP (3 sites)** | THREE stderr strings leak absolute paths, none redacted: `cli_query.py:158` (`"Error: {exc}"`, `IncompatibleIndexFormatError` branch — `exc.__str__` embeds `chroma_path` via `errors.py:58-61`); `:162` (`"failed to open index at {data_dir}: {exc}"` — leaks via BOTH the `data_dir` token AND the `{exc}` message, e.g. a `FileNotFoundError` carrying `…/chroma.sqlite3`); `:171` (`"the index at {data_dir} is empty"`). `_redact_home` lives in `cli.py:37-51`, used only at `cli.py:316,:367`. Symmetric leak on `serve`: `cli.py:271` `logger.error("%s", exc)`. |
| **R7 per-command dispatch tests** | ⚠️ **GAP** | `TestDispatch` covers only `check-deprecation` (`:75-90`), `status` (`:92-104`), `search-docs` (`:106-143`). Missing: `get-doc`, `get-example`, `search-examples`, `get-code-snippet`, `search-api`. |
